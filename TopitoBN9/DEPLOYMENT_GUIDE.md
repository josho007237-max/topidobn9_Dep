...
## 7. Deploy ด้วย Firebase Hosting + Cloud Functions

ใช้ไฟล์ `firebase.json` และโครงสร้าง `functions/` ที่เตรียมไว้เพื่อดีพลอยทั้ง Backend และ Frontend ภายใต้ Firebase เดียวกัน

| ไฟล์ | หน้าที่ |
| --- | --- |
| `firebase.json` | ตั้งค่า Hosting + rewrite ไปหา Cloud Function `app` และเพิ่ม `hosting.predeploy` เพื่อ build frontend อัตโนมัติก่อน deploy |
| `.firebaserc` | เก็บ mapping ของ Firebase project alias ที่ใช้ตอน deploy |
| `functions/index.js` | สร้าง Express app ผ่าน Cloud Functions (ปรับ `runtimeOptions` ได้) |
| `functions/package.json` | อ้างถึงแพ็กเกจ `topito-backend` เพื่อแชร์โค้ด Backend |

### 7.1 เตรียมโปรเจกต์ Firebase

1. สร้าง Firebase project (หรือเลือกโปรเจกต์ที่มีอยู่)
2. ติดตั้ง CLI: `npm install -g firebase-tools`
3. Login: `firebase login`
4. ตั้งค่าโปรเจกต์ดีฟอลต์ใน repo: `firebase use <project-id>` หรือแก้ `TopitoBN9/.firebaserc`

### 7.2 จัดการ Environment Variable

- เริ่มจากไฟล์ตัวอย่าง: `cp TopitoBN9/functions/.env.example TopitoBN9/functions/.env`
- เติมค่าเหมือนกับไฟล์ราก `TopitoBN9/.env` (เช่น `TELEGRAM_BOTS`, `DOMAIN`, `SUPABASE_URL`, `OPENAI_API_KEY`)
- Firebase CLI จะอ่านไฟล์ `.env` ภายในโฟลเดอร์ `functions/` และอัปโหลดค่าเป็น environment variables ให้ Cloud Functions โดยอัตโนมัติ
- หากต้องการใช้ Secret Manager ให้รัน `firebase functions:secrets:set KEY_NAME="value"` แล้วเพิ่มชื่อ `KEY_NAME` ลงใน `runtimeOptions.secrets` ภายใน [`functions/index.js`](./functions/index.js)

### 7.3 ติดตั้ง dependency และ build
...
> ระหว่าง `firebase deploy` จะมี predeploy ช่วยรัน `npm --prefix frontend install` และ `npm --prefix frontend run build` ให้โดยอัตโนมัติ
...
- Hosting จะดึงไฟล์จาก `frontend/dist`
- Cloud Function `app` จะให้บริการ REST API ทั้ง `/api/**`, `/webhook/**`, `/health`

### 7.5 ตรวจสอบหลัง Deploy

1. เปิด `https://<project-id>.web.app/api/system/status` เพื่อตรวจสอบสุขภาพระบบและการเชื่อมต่อ Telegram/Supabase/OpenAI
2. ทดสอบยิง webhook โดยส่ง `POST` ไปที่ `https://<region>-<project-id>.cloudfunctions.net/app/webhook/<botId>` (หรือ `/webhook/<botId>` ผ่านโดเมน Hosting)
3. เข้าหน้า Dashboard บน Firebase Hosting เพื่อตรวจสอบว่า API เรียกใช้ได้สมบูรณ์
